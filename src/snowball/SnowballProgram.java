...
public class SnowballProgram implements Serializable {
	...
	protected boolean in_grouping(char [] s, int max)
    {
	if (cursor >= limit) return false;
	char ch = current.charAt(cursor);
	if (ch > max || ch < 97) return false;
	ch -= (char) 97;
    ...
    protected boolean out_grouping(char [] s, int max)
    {
	if (cursor >= limit) return false;
	char ch = current.charAt(cursor);
	if (ch > max || ch < 97) {
    ...
    protected boolean out_grouping_b(char [] s, int min)
    {
	if (cursor <= limit_backward) return false;
	char ch = current.charAt(cursor - 1);
	if (ch > 121 || ch < min) {
    ...
    protected void insert(int c_bra, int c_ket)
    {
	int adjustment = replace_s(c_bra, c_ket, "e");
